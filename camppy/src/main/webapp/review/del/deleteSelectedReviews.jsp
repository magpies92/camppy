...
<%
String reviewIdsParam = request.getParameter("reviewIds");
if (reviewIdsParam == null || reviewIdsParam.isEmpty()) {
    out.print("empty"); // 파라미터가 없거나 비어있을 경우 "empty" 반환
} else {
    try {
        String[] reviewIdArray = reviewIdsParam.split("\\|"); // 파이프로 구분된 문자열을 배열로 분리
        ...
        // 배열을 순회하면서 각 리뷰를 삭제
        for (String reviewIdParam : reviewIdArray) {
